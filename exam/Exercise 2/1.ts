...
function A66(input) {
  ...
  let ArrayToObject = {};           // converting Array to object 

  for (let idNumber of newUniqList) {
    if (ArrayToObject[idNumber] === undefined) {
      ArrayToObject[idNumber] = 1
    } else {
      ArrayToObject[idNumber]++
    }
  }
